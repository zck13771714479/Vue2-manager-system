  ...
  <div>
    ...
    >
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="addOrUpdateConfirm('ruleForm')"
          >确 定</el-button
        >
      </div>
    </el-dialog>
...
export default {
  ...
  data() {
    var validateTmName = (rule, value, callback) => {
      if (value.length < 2 || value.length > 10) {
        callback(new Error("品牌名称长度必须在2-10个字符之间"));
      } else {
        //放行
        callback();
      }
  ...
  methods: {
    ...
    async updateTradeMark(row) {
      this.dialogFormVisible = true;
      this.type = "修改品牌";
      //不能把地址赋值过来，否则会因为地址相同，对于表单的修改在输入时（还没有确认时）会直接影响页面
      this.tmForm = { ...row };
    },
    deleteTradeMark(row) {
      //弹框
